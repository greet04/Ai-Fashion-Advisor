...
def fashion_advisor():
    ...
    # Gemini AI setup — API key must be provided via environment variable only
    API_KEY = os.environ.get("GEMINI_API_KEY")  # <<-- no hard-coded default here
    if API_KEY:
        genai.configure(api_key=API_KEY)

    try:
        if not API_KEY:
            # No API key set — return a clear message instead of calling Gemini
            bot_reply = "Gemini API key is not configured. Please set GEMINI_API_KEY in the environment to enable AI recommendations."
        else:
